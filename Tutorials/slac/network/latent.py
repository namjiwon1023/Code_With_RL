...
from slac.utils import build_mlp, calculate_kl_divergence, initialize_weight
...
class LatentModel(nn.Module):
    ...
    def sample_prior(self, actions):
        ...
        # p(z1(0)) = N(0, I)
        z1_mean, z1_std = self.z1_prior_init(actions[:, 0]) # size : [Batch_size, action_shape:6] 所有的最初动作
        z1 = z1_mean + T.randn_like(z1_mean) * z1_std
        # p(z2(0) | z1(0))
        z2_mean, z2_std = self.z2_prior_init(z1)
        z2 = z2_mean + T.randn_like(z2_mean) * z2_std
        ...
        for t in range(1, actions.size(1) + 1):     # state sequence length 9(Because have state(t + 1)), action sequence length 8(just t)
            # p(z1(t) | z2(t-1), a(t-1))
            z1_mean, z1_std = self.z1_prior(T.cat([z2, actions[:, t - 1]], dim=1))
            z1 = z1_mean + T.randn_like(z1_mean) * z1_std
            # p(z2(t) | z1(t), z2(t-1), a(t-1))
            z2_mean, z2_std = self.z2_prior(T.cat([z1, z2, actions[:, t - 1]], dim=1))
            z2 = z2_mean + T.randn_like(z2_mean) * z2_std
        ...
        return (z1_mean_list, z1_std_list)
    ...
    def calculate_loss(self, state, action, reward, done):
        ...
        z1_mean_pri, z1_std_pri = self.sample_prior(action)
