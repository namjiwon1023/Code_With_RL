        ...
        for i in tqdm(range(self.args.episode)):
            ...
            while (not done):
                ...
                if self.agent.memory.ready(self.args.batch_size):
                    q1_loss, q2_loss, critic_loss, actor_loss, alpha_loss, pg_loss, bc_loss = self.agent.learn()
                    n_updates += 1
                ...
                if self.agent.total_step % self.args.evaluate_rate == 0 and self.agent.memory.ready(self.args.batch_size):
                    ...
                    self.writer.add_scalar('Loss/Q1', q1_loss, n_updates)
                    self.writer.add_scalar('Loss/Q2', q2_loss, n_updates)
                    self.writer.add_scalar('Loss/PG', pg_loss, n_updates)
                    self.writer.add_scalar('Loss/BC', bc_loss, n_updates)
