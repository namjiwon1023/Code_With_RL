    ...
    def run(self):
        ...
        for i in range(self.args.episode):
            ...
            while (not done):
                ...
                self.agent.transition += [reward, next_state, real_done]
                one_step_transition = self.agent.memory.store(*self.agent.transition)
                if one_step_transition:
                    self.agent.memory.store(*one_step_transition)
                state = next_state
                score += reward
