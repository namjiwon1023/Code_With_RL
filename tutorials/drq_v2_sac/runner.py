...
class Runner:
    def __init__(self, agent, args, writer):
        ...
        self.episode_limit = self.env._max_episode_steps
        self.writer = writer
    ...
    def run(self):
        ...
        for i in range(self.args.episode):
            ...
            while (not done):
                ...
                if self.agent.memory.ready(self.args.batch_size) and self.agent.total_step >= self.args.init_random_steps:
                    if self.agent.total_step == self.args.init_random_steps:
                        for _ in range(self.args.init_random_steps):
